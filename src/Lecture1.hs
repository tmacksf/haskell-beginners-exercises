...
Try to use local variables (either let-in or where) to implement this function.
-}

minmax :: Int -> Int -> Int -> Int
minmax x y z = max x (max y z) - min x (min y x)
...
subString start end str
  | end < 0 = [] 
  | start < 0 = [] 
  | otherwise = reverse (drop (length str - end -1 ) (reverse (drop start str)))
...
lowerAndGreater n xs = show n ++ " is greater than " ++ show (go (>) n xs) ++ " elements and lower than " ++ show (go (<) n xs) ++ " elements" where 
  go :: (Int -> Int -> Bool) -> Int -> [Int] -> Int
  go _ _ [] = 0
  go fn n (x:xs) = if fn n x then 1 + go fn n xs else go fn n xs
